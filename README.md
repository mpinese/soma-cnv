...
This file can be generated by a number of methods, and soma-cnv should be quite robust to the choice.  soma-cnv includes a basic tool written in [nim](https://nim-lang.org/) to do this:
```bash
util/cram2ad data/truseq_nano_hiseqX_hs37d5x.loci.tsv <reference.fa> sampleID.cram | \
  xz -c > sampleID.soma-cnv.hetdp.xz
```
This tool requires the [hts-nim](https://github.com/brentp/hts-nim) and [docopt](https://github.com/docopt/docopt.nim) libraries, and can be compiled using `nim c -d:release cram2ad.nim`. Note that in some cases this compilation 
...
nim -f --gcc.exe:musl-gcc --gcc.linkerexe:musl-gcc --passL:"-static -L/usr/local/musl/lib -lhts -lz -lbz2 -llzma" --dynlibOverride:libhts c -d:release cram2ad.nim
```

Common variants callers can also be used, for example GATK HaplotypeCaller:
```bash
